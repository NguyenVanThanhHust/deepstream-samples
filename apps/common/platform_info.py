...
import cuda.bindings.runtime as cudart_runtime
    ...
    def is_integrated_gpu(self):
        ...
        with guard_platform_info:
            ...
            if not self.is_integrated_gpu_verified:
                (cuda_init_result,) = cuda_driver.cuInit(0)
                if cuda_init_result == cuda_driver.CUresult.CUDA_SUCCESS:
                    # Get cuda devices count
                    device_count_result, num_devices = cuda_driver.cuDeviceGetCount()
                    if device_count_result == cuda_driver.CUresult.CUDA_SUCCESS:
                        # If atleast one device is found, we can use the property from
                        # the first device
                        if num_devices >= 1:
                            # Get properties from first device
                            property_result, properties = (
                                cudart_runtime.cudaGetDeviceProperties(0)
                            )
                            if property_result == cuda_driver.CUresult.CUDA_SUCCESS:
