...
```
make && ./resnet_cpp /opt/nvidia/deepstream/deepstream/samples/streams/sample_720p.h264
```

Use gstreamer without cpp
For new streammux, don't define width/height for nvstreammux
```
USE_NEW_NVSTREAMMUX=yes gst-launch-1.0 filesrc location=/opt/nvidia/deepstream/deepstream/samples/streams/sample_720p.h264 name=source_0 ! decodebin ! mux.sink_0  nvstreammux name=mux batch-size=1 ! nvinfer config-file-path=nvinfer_config.txt batch-size=1 ! nvmultistreamtiler ! nvvideoconvert ! nvdsosd process-mode=0 display-text=1 ! nveglglessink
```

For old streammux: 
```
gst-launch-1.0 filesrc location=/opt/nvidia/deepstream/deepstream/samples/streams/sample_1080p_h264.mp4 name=source_0 ! decodebin ! mux.sink_0  nvstreammux name=mux batch-size=1 width=1920 height=1080 ! nvinfer config-file-path=nvinfer_config.txt batch-size=1 ! nvmultistreamtiler ! nvvideoconvert ! nvdsosd process-mode=0 display-text=1 ! nveglglessink
```
